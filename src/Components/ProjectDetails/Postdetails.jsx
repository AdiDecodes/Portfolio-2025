...
const Postdetails = () => {
  ...
  const iconsMapping = {
    ...
    tailwindcss: <RiTailwindCssFill color="#38BDF8" />, // Tailwind cyan
    ...
    styledcomponents: <SiStyledcomponents color="#DB7093" />, // Styled Components pink
    ...
    flask: <SiFlask color="#fff" />, // Flask
  };

  const handleIconMapping = (tech) => {
    const iconName = tech.toLowerCase().replace(" ", "").replace(".", "");
    for (const key in iconsMapping) {
      if (key == iconName || iconName.includes(key)) {
  ...
  useEffect(() => {
    ...
    const leftWrapper = document.querySelector(`.${styles.left}`);
    const rightWrapper = document.querySelector(`.${styles.right}`);
